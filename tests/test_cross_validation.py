...
class TestPanelSplit(unittest.TestCase):
    def setUp(self):
        ...
        self.periods = pd.Series(
            pd.date_range(start="2022-01-01", end="2022-01-10").tolist() * 2
        )
    ...
    def test_gen_train_labels(self):
        ...
        train_labels = self.panel_split.gen_train_labels(
            pd.Series(range(len(self.periods)))
        )
        pd.testing.assert_series_equal(
            train_labels, labels
        )  # Assuming one split has no train data

    def test_gen_test_labels(self):
        # Test if the gen_test_labels method returns the correct test labels
        vals = [7, 8, 9, 17, 18, 19]
        labels = pd.Series(vals, index=vals)
        test_labels = self.panel_split.gen_test_labels(
            pd.Series(range(len(self.periods)))
        )
        pd.testing.assert_series_equal(
            test_labels, labels
        )  # Assuming one split has no test data
    ...
    def test_parallel_prediction(self):
        ...
        ps = PanelSplit(
            periods=pd.Series(df.index.get_level_values("time")), n_splits=5
        )
        pred_df = ps.gen_test_labels(df.iloc[:, 0].reset_index())
        pred_df["pred"], _ = cross_val_fit_predict(
            model, X=df.iloc[:, 1:], y=df.iloc[:, 0], cv=ps
        )
        pred_df["pred_parallel"], _ = cross_val_fit_predict(
            model, X=df.iloc[:, 1:], y=df.iloc[:, 0], cv=ps, n_jobs=-1
        )

        # Assert whether the mean squared errors are equal
        self.assertTrue(
            np.isclose(
                mean_squared_error(pred_df[0], pred_df["pred"]),
                mean_squared_error(pred_df[0], pred_df["pred_parallel"]),
            )
        )
