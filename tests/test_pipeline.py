...
class TestSequentialCVPipeline(unittest.TestCase):
    ...
    def test_non_cv_pipeline(self):
        ...
        pipe = SequentialCVPipeline(
            [("double", DoubleTransformer(), None), ("dummy", DummyRegressor(), None)],
            verbose=False,
        )
    ...
    def test_fit_predict_proba(self):
        ...
        pipe = SequentialCVPipeline(
            [("double", DoubleTransformer(), None), ("dummy", DummyClassifier(), None)],
            verbose=False,
        )
    ...
    def test_passthrough_step(self):
        ...
        pipe = SequentialCVPipeline(
            [("double", None, None), ("dummy", DummyRegressor(), None)], verbose=False
        )
        pipe.fit(self.X, self.y)
        pred = pipe.predict(self.X)
