...
def check_periods(periods, obj_name="periods"):
    if isinstance(periods, pd.Series):
        ...
        raise ValueError(
            f"{obj_name} must be a level of an index. Got a pd.MultiIndex instead."
        )
        ...
        if len(periods.shape) > 1:
            raise ValueError(
                f"{obj_name} array must be one-dimensional. Got an array of shape {periods.shape} instead"
            )
        return pd.Series(periods)
    else:
...
def check_labels(labels):
    if not isinstance(labels, (pd.Series, pd.DataFrame, pd.Index, np.ndarray)):
        raise TypeError(
            f"labels object type {type(labels)} not supported. labels must be a Pandas Series, DataFrame, Index, or NumPy array"
        )
